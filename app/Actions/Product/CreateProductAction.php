...
class CreateProductAction {
    public function execute($request)
    {
        ...
        if (Product::where('name', $request->name)->first()) return response()->json(['error' => 'Product already exists!']);
        ...
        $product = Product::createProduct($request->name, $request->price, $request->discount, $price_with_discount, $request->first_img, $request->second_img, $request->category_id, $request->brand_id);
